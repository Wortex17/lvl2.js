 ...
 * Multiple glob patterns are separated by newlines (\n)
 * Example for shorthand STATIC_BLOCK (ES5)
    { "blockedDir/\nblockedDir2/": false }
 * or
 * rules[
 "blockedDir/\n" +
 ...
 * Example for shorthand STATIC_BLOCK (ES6)
 * rules[`
 blockedDir/
 blockedDir2/`] = false;
 *
 * Be advised that no trimming/cleaning of glob patterns is made.
 * Be careful not to introduce unwanted tabs or whitespaces with formatting
 *
...
function parseGlobMultiPattern(string)
{
    ...
    return cleanGlobs(raw_patterns);
}
function cleanGlobs(patternArray)
{
    var cleanPatterns = [];
